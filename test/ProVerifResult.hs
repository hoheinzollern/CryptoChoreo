...
parseResultLine = do
  _ <- string "RESULT "
  -- Try parsing with " is " first, otherwise parse until " cannot"
  result <- choice
    [ do queryText <- manyTill anyChar (try (string " is "))
         choice 
           [ string "true." >> return (QueryTrue queryText)
           , string "false." >> return (QueryFalse queryText)
           ]
    , do queryText <- manyTill anyChar (try (string " cannot be proved"))
         optional (char '.')
         return (QueryCannotProve queryText)
    ]
  optional (char '\n')
  return result
...
parseProVerifOutput output = 
  ProVerifOutput 
    { queries = simpleParseResults output  -- Use simple parser as it's more reliable
    , rawOutput = output
    }
  ...
  where
    ...
    parseSimpleLine line 
      | " is true." `isInfixOf` line = 
          QueryTrue (extractQueryWithIs line " is true.")
      | " is false." `isInfixOf` line = 
          QueryFalse (extractQueryWithIs line " is false.")
      | " cannot be proved" `isInfixOf` line = 
          QueryCannotProve (extractQueryCannotProve line)
      | otherwise = 
          QueryCannotProve (drop 7 line)  -- fallback: everything after "RESULT "
    
    extractQueryWithIs :: String -> String -> String
    extractQueryWithIs line suffix = 
      let afterResult = drop 7 line  -- drop "RESULT "
          parts = words afterResult
          beforeIs = unwords $ takeWhile (/= "is") parts
      in beforeIs
    
    extractQueryCannotProve :: String -> String
    extractQueryCannotProve line =
      let afterResult = drop 7 line  -- drop "RESULT "
          -- Find position of " cannot be proved" and take everything before it
          findSuffix [] = []
          findSuffix str@(x:xs)
            | " cannot be proved" `isPrefixOf` str = []
            | otherwise = x : findSuffix xs
      in findSuffix afterResult
